            ...
            for i in range(0, len(conv_list) - 1, 2):
                ...
                for line in action_lines:
                    ...
                    if line.startswith("pyautogui."):
                        ...
                        actions.append(UIAction(reasoning=line))
...
def parse_all_datasets(config_yaml):
    ...
    for dataset in config["datasets"]:
        ...
        # Skip regrouping for AGUVIS datasets (only regroup OS Atlas-style element lists)
        ...
        summary["datasets"][dataset_name] = {
            ...
            "skipped": skipped,
            "warnings": warnings,
            "parsed_percent": round(100 * parsed / len(data), 2) if total else 0.0,
            "skipped_percent": round(100 * skipped / len(data), 2) if total else 0.0,
            "warning_percent": round(100 * warnings / len(data), 2)if total else 0.0
        }
    ...
    #         f"({stats['parsed_percent']}%) | Skipped = {stats['skipped']} "
    #         f"({stats['skipped_percent']}%) | Warnings = {stats['warnings']} "
    #         f"({stats['warning_percent']}%)"
    #     )
...
def main():
    ...
    unified_path = f"{OUTPUT_PATH}/aguvis_parsed_unified.json"
